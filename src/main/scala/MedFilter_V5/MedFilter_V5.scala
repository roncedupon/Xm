...
class MedFilter_V5 extends Component{
    ...
    io.Idle_Now:=(Fsm.currentState===MedEnum.IDLE)
    ...
    //等一行
    Fsm.Col_End:=Col_Cnt.valid//Delay(,1)
    Fsm.Col_End_End:=Fsm.currentState===MedEnum.COl_END&&io.sData.valid
    //等全部行
    ...
    val mem=Array.tabulate(8)(i=>{
        def gen():Mem[UInt]={
            ...
            if(i<4){//处理0,2,4,6的ram---
                ...
                mData_flow(5*i):=RegNext(RdData(2*i)(15 downto 0))//低16位为双数
                mData_flow(5*i+25):=RegNext(RdData(2*i)(31 downto 16))//高16位为双数
            }
            mem
        }
        gen()
    ...
    //开始掐头去尾
    val Out_Data_Valid_Bram=RegNext(io.sData.valid&&Fsm.currentState===MedEnum.WAIT_LAST_ROW)
    ...
    Delay_valid(1):=Delay(valid(1),1)//需要注意的是,
    ...
    when(Fsm_Watched===MedEnum.WAIT_LAST_ROW){
        when(Out_Data_Col>3&&Out_Data_Col<1024){
            valid(0):=Out_Data_Valid
        }otherwise{
            valid(0):=False
        }
        when(Out_Data_Col>2&&Out_Data_Col<1024-1){
            valid(1):=Out_Data_Valid
        }otherwise{
            valid(1):=False
        }
        when(Out_Data_Col>1&&Out_Data_Col<1024-2){
            valid(2):=Out_Data_Valid
        }otherwise{
            valid(2):=False
        }
        when(Out_Data_Col>0&&Out_Data_Col<1024-3){
            valid(3):=Out_Data_Valid
        }otherwise{
            valid(3):=False
        }
        when(Out_Data_Col<1024-4){
    ...
    for(i<-0 to 49){
        ...
        Fifo(i).io.fifo_ready:=Fifo(0).io.Pop_Valid//走Flow流,下一层一直准备好得了
    }
    io.mValid:=Delay(Delay_valid(0),2)
	Fsm.Fsm_Fifo_Ready:=Fifo(0).io.Data_In.ready
	io.sData.ready:=(Fsm.currentState===MedEnum.WAIT_9_ROWS||Fsm.currentState===MedEnum.WAIT_LAST_ROW)&&Fifo(0).io.Data_In.ready//Fifo_0.io.Data_In.ready//

}
object MemGen extends App { 
    val verilog_path="./testcode_gen/MemGen" 
   SpinalConfig(targetDirectory=verilog_path, defaultConfigForClockDomains = ClockDomainConfig(resetActiveLevel = HIGH)).generateVerilog(new MedFilter_V5)
}
