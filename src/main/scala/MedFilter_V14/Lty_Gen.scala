...
//状态机相关====================================================================
    ...
    val Bram_Out_Cnt=WaCounter(io.mData2.fire, log2Up(Config.LTY_DATA_BRAM_B_DEPTH), Config.LTY_DATA_BRAM_B_DEPTH-1)//创建输出数据的列计数器
...
//一个Bram写选择器，一个Bram读选择器
    val Bram_Read_Chose=Reg(UInt(1 bits))init(0)//要准确控制才行，是读0，1还是2，3
    //4个Bram的作用是在计算前两行的时候加载后两行数据
    when(Bram_Out_Cnt.valid){
        Bram_Read_Chose:=Bram_Read_Chose+1
    }
...
//建立四个数据缓存Bram=======================================================================
    ...
    io.mData1.payload:=Bram_Read_Chose.mux(//输出数据选择器
        0->FeatureMem(0).io.doutb,
        1->FeatureMem(2).io.doutb
    )
    io.mData2.payload:=Bram_Read_Chose.mux(//输出数据选择器
...
//控制数据输入输出
    ...
    when(Fsm.currentState===LTY_ENUM.EXTRACT_LTY||Fsm.currentState===LTY_ENUM.WAIT_NEXT_READY){//这里加RegNext的原因：对波形时开头多了一个点结尾少了一个点，待分析
