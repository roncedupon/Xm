...
//方案三============================================
	...
	//开始掐头去尾======================================================
	val Start_Send=Reg(Bool())init(False)//开始发送
	val Stop_Send=Reg(Bool())init(False)//停止发送,数完一行就停下来
	...
	val Wait_5_Cnt=WaCounter(Fsm_Qtqw.currentState===Wait5Enum.WAIT_5&&Fsm.currentState===MedEnum.WAIT_LAST_ROW,3,3)//创建列计数器,这里是4还是5是看仿真看出来的....
																													//更新:好像得是3...
	Start_Send:=Wait_5_Cnt.valid//开始发送:数完x拍后开始发送
		...
		def gen():Mem[UInt]={//在循环中定义一个函数,这个函数在每次循环都会返回一个UInt的mem
			...
			if(i<4){//处理0,2,4,6的ram---
				...
				//5,6,7,8,9
					...
					// }
					mData_flow(j+5*i):=RegNext(mData_flow(j+5*i-1))//RegNext()//,Fsm_Qtqw.currentState===Wait5Enum.DATA_VALID)
					mData_flow(j+5*i+25):=RegNext(mData_flow(j+5*i-1+25))//RegNext()//,Fsm_Qtqw.currentState===Wait5Enum.DATA_VALID)
				}//
	...
	mData_flow(20):=io.sData.payload(15 downto 0)//RegNext()//RegNext()
	...
	mData_flow(45):=io.sData.payload(31 downto 16)//RegNext()//RegNext()
	for(i<-0 to 49){
