...
//仿真lite口用
case class Axi_Lite_RWF_Fsm(Axi_Lite_Port : AxiLite4, clockDomain : ClockDomain) extends Area {
	...
	switch(currentState) {
		...
		is(Axi_Lite_Ctrl.READ_LITE){
			...
			// 	Axi_Lite_Port.ar.valid := False
			// }else
			when(Axi_Lite_Port.ar.ready&&Axi_Lite_Port.r.valid&&Axi_Lite_Port.r.payload.data===B"32'hf"){
				nextState:=Axi_Lite_Ctrl.SWITCH_STATE
...
case class AxiLite_Control_RWF() extends Component{
	val IO=new Bundle{
		val Inst_In=in Bits(64 bits)
		val Start_Signal=in Bool(1 bits)
		val Axi_Lite_Port=master(AxiLite4(32, 32))
		val Read_Inst_En=out Bool()
	}
	noIoPrefix()
